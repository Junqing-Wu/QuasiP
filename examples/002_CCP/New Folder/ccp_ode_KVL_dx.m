function dx_dx = ccp_ode_KVL_dx(t, x, p)
    %% Params
    ...
    N = numel(Qs1);
    dx_dx = zeros(7, 7, N);
    dx_dx(1,1,:) = -1./(Cm1 .* par.Rrf);
    dx_dx(1,2,:) =  1./(Cm1 .* par.Rrf);
    dx_dx(2,4,:) = 1;
    dx_dx(3,5,:) = 1;
    dx_dx(4,1,:) = 1./(Cm1 .* par.Lm2);
    dx_dx(4,2,:) = -(1./par.Cstray + 1./Cm1 + 1./Cm2)./par.Lm2;
    dx_dx(4,3,:) = 1./(par.Cstray .* par.Lm2);
    dx_dx(4,4,:) = -(par.Rm + par.Rstray)./par.Lm2;
    dx_dx(4,5,:) = par.Rstray ./ par.Lm2;
    dx_dx(5,2,:) = 1./(par.Cstray .* par.Lpl);
    dx_dx(5,3,:) = -1./(par.Cstray .* par.Lpl);
    dx_dx(5,4,:) = par.Rstray ./ par.Lpl;
    dx_dx(5,5,:) = -(par.Rstray + par.Rpl)./par.Lpl;
    dx_dx(5,6,:) = -Qs1./(par.Cs10^2 .* par.Te .* par.Lpl);
    dx_dx(5,7,:) = Qs2./(par.Cs20^2 .* par.Te .* par.Lpl);
    dx_dx(6,5,:) = 1;
    arg1 = -(Qs1./(sqrt(2)*par.Cs10*par.Te)).^2;
    dx_dx(6,6,:) = par.Ie10 .* (-Qs1./(par.Cs10^2 .* par.Te^2)) .* exp(arg1);
    dx_dx(7,5,:) = -1;
    arg2 = -(Qs2./(sqrt(2)*par.Cs20*par.Te)).^2;
    dx_dx(7,7,:) = par.Ie20 .* (-Qs2./(par.Cs20^2 .* par.Te^2)) .* exp(arg2);
